...
def fractional_knapsack(wgt: list[int], val: list[int], cap: int) -> int:
    # calculate ratio(val/wgt) of each items and sort by ratio
    items = [{"w": w, "v": v, "r": v/w} for w, v in zip(wgt, val)]
    items.sort(key=lambda x: x["r"], reverse=True)
    # count how many items and total value in the snapack
    value = 0
    for item in items:
        if item.get("w") <= cap:
            value += item.get("v")
            ...
            value += item.get("r")*cap
            break
    return value


wgt = [20, 40, 10, 30, 50]
val = [120, 210, 50, 150, 240]
fractional_knapsack(wgt=wgt, val=val, cap=50)